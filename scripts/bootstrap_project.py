...
def ensure_coldstart_handoff(workspace: Path) -> Path:
    ...
    body = "\n".join(
        [
            ...
            "- Domain: shared memory workflow for two paired assistants with role-driven planning, execution, review, and fix ownership",
            ...
            "- Major request changes: stop binding strong/weak roles to fixed model names, use request entry to determine the primary assistant, require more detailed plans, and keep the workflow synchronized with the local context-coldstart-pack skill",
            "- Decision sequence: initialize shared-memory MCP, split primary planning/review from secondary execution/fix, require detailed primary-lane planning, then keep durable state in Markdown files plus `.ai-pair/`",
            "- Important turning points: medium and large changes must pass through OpenSpec before Kimi executes",
            "",
            "## Persistent Context",
            "",
            "- Stable terminology: the assistant that receives the request first becomes the primary assistant; the paired assistant becomes the secondary assistant for that change",
            ...
            "- Workflow and code-style constraints: read both durable memory files before substantive work, keep them synchronized after meaningful rounds, and write plans detailed enough that the primary assistant could execute them directly",
            "",
            "## Current State",
            "",
            ...
            "- Latest next step: connect both assistants to the shared-memory workflow, assign the first request recipient as the primary assistant, then start the first planned change",
            ...
            "- Primary/secondary ownership model defined",
            ...
            "- Workflow order: primary assistant plan/review -> secondary assistant execute/fix -> primary assistant final review",
            ...
            "- Exact next step: decide which assistant is primary for the next requested change, then decide whether OpenSpec is needed before handing work to the secondary assistant",
